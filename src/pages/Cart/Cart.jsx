...
import EmptyCart from '~/assets/svg/empty-cart.svg?react'
import { useSelector, useDispatch } from 'react-redux'
import { API_GetProductById } from '~/apis/index'
...
function Cart({ info }) {
	...
	useEffect(() => {
		const fetchProducts = async () => {
			const promises = productInCart.map(item => {
				return API_GetProductById(item.id)
					.then(data => ({
						...data,
	...
	let totalPrice
	if (productList) {
		totalPrice = productList.map(product => (product.price - (product.price * product.precent / 100)) * product.quantityInCart).reduce((init, curr) => {
			return init + curr
		}, 0)
