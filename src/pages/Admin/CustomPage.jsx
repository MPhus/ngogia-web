...
const CustomPage = memo(({ type }) => {
	...
	// 	data.append('pageId', otherPage?._id)
	// 	data.append('webId', otherPage?.webId)
	...
	// 	setOtherPage(newOtherPage)
	// 	moveNewsOnOtherPage_API(slug, { newListOderIds: newsOrderedIds })
	// }
	...
	// 	setOtherPage(newOtherPage)
	// 	deleteNewsOnOtherPage_API(slug, newsId)
	// 		.then(data => {
			...
			<Box sx={{ backgroundColor: '#fff', padding: '40px' }}>
					...
					<Box>
						...
						<Box>
							...
							<Box sx={{
								...
								<SettingProductSlide thumb={storeTop} updateSlide={updateSlide} slug={slug} />
							</Box>

							<Box>
								<Typography variant='h4'>Chỉnh sửa hình ảnh trang Pant</Typography>
								<SettingProductSlide thumb={storeBottom} isBottom updateSlide={updateSlide} slug={slug} />
							</Box>
				...
				{otherPage &&
					<Box>
						...
						<SettingSlide page={otherPage} ortherPage updateSlide={updateSlide} slug={slug} />

						<CustomOrtherPage page={otherPage} addNews={addNews} moveNews={moveNews} deleteNews={deleteNews} updateNews={updateNews} slug={slug} />
					</Box>
				} */}
			</Box>
