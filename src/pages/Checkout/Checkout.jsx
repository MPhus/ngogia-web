...
function Checkout({ slug, webInfo }) {
	...
	const totalPrice = productList.map(product => (product.price - (product.price * product.precent / 100)) * product.quantityInCart).reduce((init, curr) => {
		return init + curr
	}, 0)
				...
				<Box sx={{
					...
					<Box sx={{
						...
						<Box sx={{
							...
							<Link to={`/`}>
								<img src='https://res.cloudinary.com/divrizdwo/image/upload/v1761395638/NGSN-Logo_x751zj.png' alt='NgoGia logo' />
							</Link>
